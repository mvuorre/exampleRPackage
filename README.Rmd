...
You will find all the raw data as .csv files in the [`data-raw/`](https://github.com/mvuorre/exampleRPackage/tree/master/data-raw) folder, from where they can be downloaded. That folder also contains full pre-processing instructions in the form of an R script.

## How to obtain the data from within R

One major benefit of wrapping the data into an R package is its easy availability from within R. No more wasted time looking around the computer's hard drive for the correct data files!

Because the exampleRPackage is hosted on GitHub, you can easily download it from within R:
...
The rest of this README file describes briefly how to create R packages. In writing this online tutorial, we relied heavily on Hadley Wickham's "R Packages", which is an excellent source of information on creating R packages [@wickham_r_2015].
...
6. [Document data analysis as package vignette](#documenting-analysis-as-package-vignette)
    - Creates a readable .html file showing how the data is (or could be) analyzed
7. [Create a website for the data package](#creating-a-website-for-the-data-package)
...
The **devtools** package [@wickham_devtools:_2017] contains helpful functions for creating the R data package.
...
Then, you'll want to add a `README` file which describes the package in some detail. We recommend writing the file in Markdown[^markdown] or R Markdown [@allaire_rmarkdown:_2016]. Here, we choose to create a `README.Rmd` R Markdown file, which, when knitted, produces a nice looking `README.md` Markdown file. Using devtools:
...
You can then put all the raw data files to `data-raw/`. For this exampleRPackage, we simulated some simple example data, and therefore also put the R script that simulated the data and created the data files in the same folder.
...
To document your data set, create a file called `data.R` in the `R` folder. Then, use the (roxygen2 [@wickham_roxygen2:_2017]) documentation syntax to write your data object's documentation in the `R/data.R` file. It will look something like the following for our `exampleData` object:
...
The easiest way to share the data product is to create the R package as a Git repository (see our tutorial on Git + GitHub: <https://github.com/mvuorre/reproguide-curate> [@vuorre_curating_2017]) and share it on GitHub. Once the R package's source code is pushed to GitHub, authorized users (by default, anyone) can browse it on GitHub and manually download any files they'd like (e.g. the raw data files.) Importantly, they can obtain the data very easily from within R by installing the R package you have just created:

```{r, eval = FALSE}
devtools::install_github("mvuorre/exampleRPackage")
```

The above command, when executed in R, downloads and installs the `exampleDataPackage` from GitHub user `mvuorre`. You can view this example data package on GitHub: <https://github.com/mvuorre/exampleRPackage>.
...
You can even create a website for the data package. For this, you need the [pkgdown](https://hadley.github.io/pkgdown/) R package [@wickham_pkgdown:_2017]:
